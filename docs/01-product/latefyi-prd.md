...
**Version:** 1.12.0
**Status:** Phases 1–7 shipped and live at `late.fyi`. Allowlist is **open** to anyone. Privacy contract is literal: trip ends → address AND record both deleted, no archive. Disambiguation completion (numbered/named replies) wired end-to-end with `Reply-To` threading. Abuse limits live (10/hr, 50/day, 20 active per sender), deliverability verified PASS to Gmail (SPF/DKIM/DMARC), `On: <date>` advance planning live (up to 90 days). Landing page at `late.fyi` via Cloudflare Pages. **Routable From:** per-template (`<TRAINNUM>@`, `stop@`, `config@`, `help@`) with display name `latefyi`, so Reply-STOP works as a silent fallback alongside the canonical one-click `mailto:` stop links. **Subject inbox-grouping signals** — `[trip]` prefix + ISO date suffix render conditionally on confirmation and per-event subjects so multi-train trips and advance-planned trips group cleanly in the inbox. **Operator metrics**: daily snapshot (`scripts/stats.sh`) + weekly digest email (`scripts/stats-email.sh`) — aggregate counters only, no per-user/per-trip detail. **v1.12.0 bug fixes**: push notifications were silently never delivered (run() dropped transport param), bare STOP to stop@ produced "Stopped tracking null" (now STOP ALL), stats.sh exposed plaintext sender in shell pipeline (now uses senderHash), CHANNELS email reply missing FOOTER and wrong From:. 251/251 tests pass.
