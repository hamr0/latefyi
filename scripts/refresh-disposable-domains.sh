#!/usr/bin/env bash
# Refresh config/disposable-domains.txt from the upstream
# disposable-email-domains repo. Run this manually (or via quarterly cron)
...
#   git diff config/disposable-domains.txt   # review what changed
...
DEST="$HERE/config/disposable-domains.txt"
TMP="$(mktemp)"
trap 'rm -f "$TMP"' EXIT
